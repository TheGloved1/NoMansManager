    ...
    (async () => {
      try {
        ...
        id = setInterval(async () => {
          try {
            const c = await loadConfigNative();
            apply(c);
          } catch {}
        }, 1000);
      } catch {}
...
<div class="flex h-screen bg-background text-foreground overflow-hidden">
  <aside
    class="shrink-0 flex flex-col bg-card border-r transition-all duration-200 {collapsed
      ? 'w-[56px] items-center'
      : 'w-[220px]'}"
  >
    <div
      class="h-12 flex items-center gap-2 px-3 border-b shrink-0 w-full {collapsed
        ? 'justify-center'
        : ''}"
    >
      <div
        class="h-7 w-7 rounded-md bg-primary flex items-center justify-center text-primary-foreground font-black text-[11px] shrink-0"
      >
        NMM
      </div>
      {#if !collapsed}
    ...
    <nav class="flex-1 flex flex-col gap-1 w-full p-1.5 overflow-y-auto">
      <a
        href="/"
        title="Mods"
        class="flex items-center gap-2.5 rounded-md px-2.5 py-2 text-sm transition {isActive(
          '/',
        )
          ? 'bg-primary text-primary-foreground'
          : 'text-muted-foreground hover:bg-muted hover:text-foreground'} {collapsed
          ? 'justify-center px-1'
          : ''}"
      >
