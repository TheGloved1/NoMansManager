...
<div class="flex h-screen bg-background text-foreground overflow-hidden">
  <aside class="shrink-0 flex flex-col border-r bg-gradient-to-b from-card to-background transition-all duration-200 {collapsed ? 'w-[56px] items-center' : 'w-[220px]'}">
    <div class="h-12 flex items-center gap-2 px-3 border-b shrink-0 w-full {collapsed ? 'justify-center' : ''}">
      <div class="h-7 w-7 rounded-md bg-primary flex items-center justify-center text-primary-foreground font-black text-[11px] shrink-0 shadow-lg shadow-primary/25">NMM</div>
      {#if !collapsed}
    ...
    <nav class="flex-1 flex flex-col gap-1 w-full p-1.5 overflow-y-auto">
      ...
      <a href="/bases" title="Bases" class="flex items-center gap-2.5 rounded-md px-2.5 py-2 text-sm transition {isActive('/bases') ? 'bg-primary text-primary-foreground shadow-md shadow-primary/20' : 'text-muted-foreground hover:bg-muted hover:text-foreground'} {collapsed ? 'justify-center px-1' : ''}">
        <span class="text-sm leading-none shrink-0">🛰</span>
        {#if !collapsed}<span>Bases</span>{/if}
      </a>
      <a href="/logs" title="Logs" class="flex items-center gap-2.5 rounded-md px-2.5 py-2 text-sm transition {isActive('/logs') ? 'bg-primary text-primary-foreground shadow-md shadow-primary/20' : 'text-muted-foreground hover:bg-muted hover:text-foreground'} {collapsed ? 'justify-center px-1' : ''}">
        <span class="text-sm leading-none shrink-0">📋</span>
